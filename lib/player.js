...
Player.prototype.equip = function (itemName) {
    ...
    if (item.type === '2 Handed') {
        oldEquipment.push(this.equipment.filter(e => e.type === 'Main Hand'));
        oldEquipment.push(this.equipment.filter(e => e.type === 'Off Hand'));
    }
    else if(item.type === 'Main Hand' || item.type === 'Off Hand') {
        oldEquipment.push(this.equipment.filter(e => e.type === '2 Handed'));
    }
    
    oldEquipment.push(this.equipment.filter(e => e.type === item.type));
...
Player.prototype.unEquip = function (itemName) {
    const report = [];
    const item = this.equipment.find(e => e.name.toUpperCase().includes(itemName.toUpperCase()));
